...
void handle_entry(t_mf mf, t_entry_point_command *lcmd)
{
	...
	epc.entryoff = new_entry;//Our code will be at the end of the file
...
void handle_segment(t_mf mf, t_segment_command_64 *lcmd)
{
	...
	if (!ft_strncmp(newseg.segname, SEG_DATA,ft_strlen(SEG_DATA)))
	{
		...
		for (uint32_t i = 0; i < newseg.nsects; i++)
		{
			...
			if (i == newseg.nsects - 1) //we'll infect the last __DATA section
			{
				new_entry = new_section.offset + new_section.size;
				new_section.size += pad + SHELLCODE_LEN + ft_strlen(mf.key);	
			}
			write(mf.wfd, &new_section, sizeof(t_section));
...
int hack(t_mf mf)
{
	...
	write(mf.wfd, (unsigned char *)mf.file + new_start, (new_entry - new_start));// writing stuff between __text and last __data
